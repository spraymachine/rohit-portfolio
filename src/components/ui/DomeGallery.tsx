...
const DEFAULT_IMAGES: ImageItem[] = [
  ...
  {
    src: "https://images.unsplash.com/photo-1456926631375-92c8ce872def?w=800&q=80",
    alt: "Migration",
  },
];
...
// CSS styles as a constant string (safe - no user input)
const DOME_CSS = `
  .sphere-root {
...
export default function DomeGallery({
  ...
  padFactor = 0.25, overlayBlurColor = "#000000", maxVerticalRotationDeg = DEFAULTS.maxVerticalRotationDeg,
    ...
    onDrag: ({ event, last, velocity: velArr = [0, 0], direction: dirArr = [0, 0], movement }) => {
      ...
      if (last) {
        ...
        if (!isTap && Math.abs(vx) < 0.001 && Math.abs(vy) < 0.001 && Array.isArray(movement)) {
          vx = (movement[0] / dragSensitivity) * 0.02; vy = (movement[1] / dragSensitivity) * 0.02;
        }
  ...
  useEffect(() => {
    ...
    const close = () => {
      ...
      if (!originalPos) {
        ...
        focusedElRef.current = null; rootRef.current?.removeAttribute("data-enlarging"); openingRef.current = false;
        return;
      }
      ...
      if (originalImg) { const img = originalImg.cloneNode() as HTMLImageElement; img.style.cssText = "width:100%;height:100%;object-fit:cover;"; anim.appendChild(img); }
      overlay.remove(); rootRef.current!.appendChild(anim);
      void anim.getBoundingClientRect();
      requestAnimationFrame(() => {
        anim.style.left = origRel.left + "px"; anim.style.top = origRel.top + "px";
        anim.style.width = origRel.width + "px"; anim.style.height = origRel.height + "px"; anim.style.opacity = "0";
      });
  ...
  return (
    <>
      ...
      <div ref={rootRef} className="sphere-root relative w-full h-full"
        style={{ "--segments-x": segments, "--segments-y": segments, "--overlay-blur-color": overlayBlurColor, "--tile-radius": imageBorderRadius, "--enlarge-radius": openedImageBorderRadius, "--image-filter": grayscale ? "grayscale(1)" : "none" } as React.CSSProperties}>
            ...
            <div ref={sphereRef} className="sphere">
              {items.map((it, i) => (
                <div key={`${it.x},${it.y},${i}`} className="sphere-item absolute m-auto" data-src={it.src} data-alt={it.alt} data-offset-x={it.x} data-offset-y={it.y} data-size-x={it.sizeX} data-size-y={it.sizeY}
                  style={{ "--offset-x": it.x, "--offset-y": it.y, "--item-size-x": it.sizeX, "--item-size-y": it.sizeY, top: "-999px", bottom: "-999px", left: "-999px", right: "-999px" } as React.CSSProperties}>
